...
On xxxxxxx we copy these bash scripts to ~/bin/. and set the cronjob as follows:
    ...
    */5 * * * * /home/mediaimport/bin/[production|development]/run_mediainfo.sh 2>&1 | tee -a /home/mediaimport/[logs|logs-dev]/mediaInfo.$(date +"\%Y-\%m-\%d").log
    
      
    
    # Run mediaImport on ALL files once a day at 10am
    
    0 10 * * * /home/mediaimport/bin/[production|development]/run_mediainfo_all_movies.sh 2>&1 | tee -a /home/mediaimport/[logs|logs-dev]/mediaInfo.ALLMOVIES.$(date +"\%Y-\%m-\%d").log
